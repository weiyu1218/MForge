...
- 现有代码没有 `fto` loss key。
- 现有数据 loader 没有 FTO hard negative / patent negative pair 数据契约。
- 该命令可以训练 HUMU joint model，但不能声明为已接入 FTO loss 的 foundation model。
...
- FTO loss 接入未实现。
- 解决方式：
  1. 定义 `data.fto_negative_source` 数据契约，至少包含 anchor molecule、patent molecule 或 patent embedding、patent id、claim evidence、risk/distance 标签。
  2. 在 `humu_pretrain.data_loader` 中加载并 collate FTO negatives。
  3. 在 `humu_pretrain.pipeline._compute_losses()` 中增加 `l_fto`，使用 HUMU 距离 margin 或 patent dead-zone repulsive potential。
  4. 在配置中增加 `loss_weights.fto`。
  5. 用 D5 产出的真实 patent index 验证 FTO negatives 来源，不能用随机负样本代替。
...
### D5. SureChEMBL / USPTO / Reaxys 真实接入

当前仓库入口：

- `pipelines/patent_indexing/src/patent_indexing/pipeline.py::run(config)`
- `pipelines/reaction_indexing/src/reaction_indexing/pipeline.py::run(config)`
- `data/ingestion/surechembl/daily_sync.py`
- `services/fto-patent-svc/src/fto_patent_svc/infra/patent_data_sources/`

当前不能给出真实后台接入命令。原因：

- `patent_indexing.run(config)` 需要调用方注入 `vector_client` 和 `dead_zone_updater`，仓库没有命令行入口把它们从环境构造出来。
- `reaction_indexing.run(config)` 需要 `source_paths["uspto"]`、`source_paths["pistachio"]`、`source_paths["reaxys"]` 等真实本地数据路径。
- `data/ingestion/surechembl/daily_sync.py` 明确是 placeholder，返回 `PLACEHOLDER: SureChEMBL credentials not configured`。
- `fto-patent-svc` 的 `AnalyzeMolecule` 和 `CheckDeadZone` 真实 backend 尚未配置。

未完成项：

- SureChEMBL 凭证/API 或授权 dump 未接入。
- USPTO patent bulk / PatentsView 数据同步未接入到 patent index CLI。
- Reaxys 需要商业授权和法务确认，当前只有 client 骨架。
- patent vector client、dead-zone updater、`PATENT_INDEX_URI` 产物生成链路未封装成可后台运行的命令。

解决方式：

1. 法务确认 SureChEMBL / USPTO / Reaxys 数据许可和使用范围。
2. 提供真实数据路径或 API 凭证，不用 placeholder client 伪造成功。
3. 实现 patent indexing CLI：从环境构造 vector client、dead-zone updater，调用 `patent_indexing.run(config)`。
4. 实现 reaction indexing CLI：从环境读取 USPTO / Pistachio / Reaxys 本地路径，调用 `reaction_indexing.run(config)`。
5. 输出可被服务读取的 `PATENT_INDEX_URI`，再接入 D4 preflight。

### D6. Enamine REAL 49B Faiss 索引
...
- `faiss_indexer.py` 明确是 placeholder，返回 `PLACEHOLDER: Enamine REAL license not configured`。
- `supply-oracle-svc` 当前只支持 `SUPPLY_CATALOG_URI=file://...` 的本地 JSON 精确查找。
- 仓库没有 49B 规模流式 SMILES 读取、ECFP 批处理、Faiss IVF-PQ 训练、分片 add、索引落盘和 ANN 查询服务接线。

未完成项：

- Enamine REAL 49B 数据授权和实际数据文件未接入。
- 49B 规模 Faiss/IVF-PQ 构建器未实现。
- supply oracle 未接入 Faiss ANN client。

解决方式：

1. 确认 Enamine REAL 数据许可，提供真实数据路径和字段格式。
2. 实现流式 reader，按批生成 ECFP 或项目指定 embedding。
3. 实现 Faiss IVF-PQ train/add/save，输出 index 文件、id map、manifest。
4. 在 supply oracle 中增加 Faiss client，配置 `SUPPLY_CATALOG_URI` 或新增明确的 index URI 环境变量。
5. 增加小样本索引单测和真实数据 dry-run，再启动 49B 全量后台索引。

## 4. 未完成项汇总

| 项 | 未完成内容 | 解决路径 |
|---|---|---|
| D3 | HUMU FTO loss | 先由 D5 产出真实 patent negatives，再接 `loss_weights.fto` 和 `l_fto` |
| D4 | KRAS G12C 真实业务跑通 | 补齐 preflight 所有 artifact、服务和 DKI 环境 |
| D5 | SureChEMBL / USPTO / Reaxys 真实数据接入 | 法务/API/数据路径确认后，实现 patent/reaction indexing CLI 和 `PATENT_INDEX_URI` 产物 |
| D6 | Enamine REAL 49B Faiss 索引 | 数据授权后实现 49B 流式索引构建器和 supply oracle Faiss 查询接线 |
