...
def test_model_weights_path() -> None:
    assert (
        model_weights_path("humu-encoder", "v2")
        == "models/humu-encoder/v2/weights/"
    )
...
def test_audit_log_path() -> None:
    assert (
        audit_log_path("2026-05-04", "run-001")
        == "audit_logs/2026-05-04/run-001/signed.jsonl"
    )
...
def test_xdl_protocol_path() -> None:
    assert (
        xdl_protocol_path("mol-001", "route-1")
        == "xdl_protocols/mol-001/route-1.xdl"
    )
...
async def test_minio_client_put_get_and_hash() -> None:
    ...
    assert await client.object_sha256("artifacts/mol.sdf") == hashlib.sha256(
        payload
    ).hexdigest()
