"""DiffDock-L oracle: L2 docking with equivariant diffusion models."""
import inspect

from mf_core.plugins.oracle import OraclePlugin


class DiffDockLOracle(OraclePlugin):
    ...
    async def evaluate(
        ...
        if self.runner is None:
            raise RuntimeError("DIFFDOCK_RUNNER is required")
    ...
    async def predict_with_uncertainty(self, molecules, properties):
        if self.runner is None or not hasattr(self.runner, "predict_with_uncertainty"):
            raise RuntimeError("DIFFDOCK uncertainty runner is required")
    ...
    for smiles, values in result.items():
        for field in fields:
            if field not in values or values[field] in ("", None):
                raise RuntimeError(
                    f"DiffDock result for {smiles} requires {field}"
                )
