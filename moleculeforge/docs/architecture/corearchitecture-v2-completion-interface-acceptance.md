...
- 2026-06-10（H11）：完成 KRAS G12C full pilot 真实端到端验收。前置：H1 DKI、H2 Sigstore/Rekor、H5 oracle command wrapper、H6 retrosyn 四引擎与 service ready 均在 `.env`/runner 环境中通过 full preflight；`SIGSTORE_IDENTITY_TOKEN` 由 GitHub Actions OIDC 运行时注入并在日志中 mask 为 `***`，未写入 `.env`，`SIGSTORE_EXPECTED_IDENTITY` 指向 `https://github.com/weiyu1218/MForge/.github/workflows/h11-kras-e2e.yml@refs/heads/feature/corearchitecture-v2-completion`。修复提交：`2cc7e0a`（`fix: 修复H11 full critic与逆合成验证`），H11 workflow run `27275216898`，job `80554665740`，runner `mforge-h2-audit`，commit `2cc7e0afbb6d24cdacd62e0876c034ca4b954b8b`。正式验收命令由 workflow 执行：source `.env` 后导出 `RUN_KRAS_G12C_E2E=1`、`KRAS_E2E_SCOPE=full`、`PYTHONDONTWRITEBYTECODE=1`、`PYTHONUNBUFFERED=1`，运行 `uv run pytest tests/e2e/test_kras_g12c_pilot.py -q -ra`；`PROVENANCE_STORE_MODE=production_real`、DKI、Sigstore、HFM/Boltz/AiZynth 等 full-scope preflight 均满足。结果：GitHub Actions conclusion `success`，job exit code 0，pytest 日志显示 `Running 6 items in this shard`、`.....` + `.` 到 `[100%]`，即 6 项全通过、0 skipped、0 failed；仅有 LangGraph `LangChainPendingDeprecationWarning`。关键证据：`test_generates_diverse_candidates`、`test_oracle_cascade_validates_affinity`、`test_retrosyn_plans_synthesis`、`test_critic_reviews_concerns` 与 `test_end_to_end_kras_g12c` 在 full scope 下非 skip 通过；H11 full pilot 已闭环。剩余 gate：H11 无；H10 集群发布验证如需独立记录，仍归 H10 日志闭环。C1/C2/C3 无变更。
