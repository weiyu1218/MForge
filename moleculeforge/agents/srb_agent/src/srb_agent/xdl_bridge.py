...
def _fallback_xml(ssp) -> str:
    ...
    for mat in ssp.materials:
        if mat.id not in seen_r:
            seen_r.add(mat.id)
            SubElement(reagents_el, "Reagent", id=mat.id, smiles=mat.smiles,
                       quantity=str(mat.quantity), unit=mat.unit)
    ...
    op_map = {"add": "Add", "stir": "Stir", "heat": "Heat", "cool": "Cool",
              "filter": "Filter", "wash": "Wash", "dry": "Dry", "quench": "Quench",
              "evaporate": "Evaporate", "extract": "Extract", "separate": "Separate",
              "crystallize": "Crystallize", "purify": "Purify", "analyze": "Analyze",
              "reflux": "Reflux", "distill": "Distill", "degas": "Degas"}
    for step in ssp.steps:
        tag = op_map.get(step.operation, "Stir")
        temp = step.temperature_C or 25.0
        time_h = step.time_h or 2.0
        SubElement(proc_el, tag, vessel="reactor_0",
                   temp=str(temp), duration_h=str(time_h))
